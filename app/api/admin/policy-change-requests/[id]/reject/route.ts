  ...
  try {
    ...
    if (client && transactionStarted) {
      await client.query("ROLLBACK").catch((rollbackError) => {
        console.error(
          "Policy rejection rollback failed:",
    ...
    if (error instanceof ActionAutonomyPolicyChangeRequestStateError) {
      return Response.json({ errors: [error.message] }, { status: 409 });
    }

    console.error(
      "Policy change rejection failed:",
      error instanceof Error ? error.message : "Unknown error"
    );
    return Response.json(
      { errors: ["Policy change rejection failed unexpectedly."] },
      { status: 500 }
    );
  } finally {
    client?.release();
